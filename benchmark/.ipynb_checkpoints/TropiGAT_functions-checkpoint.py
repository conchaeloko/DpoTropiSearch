...
def make_ensemble_TropiSAGE_r(path_ensemble) : 
	"""
	This function builds a dictionary with all the models that are part of the TropiSAGE predictor
	Input : Path of the models
	Output : Dictionary
	# Make a json file with the versions of the GNN corresponding to each KL types
	# Load it
	...
	for GNN_model in os.listdir(path_ensemble) :
		if GNN_model[-2:] == "pt" :
			KL_type = GNN_model.split(".")[0]
			try :
				model = TropiGAT_models.TropiGAT_small_sage_module(hidden_channels = 1280, dropout = 0)
				model.load_state_dict(torch.load(f"{path_ensemble}/{GNN_model}"))
				dico_ensemble[KL_type] = model
			except Exception as e :
				a = (KL_type , e)
